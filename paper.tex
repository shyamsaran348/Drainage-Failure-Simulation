...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.48\textwidth]{results/figures/final_system_architecture.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.48\textwidth]{results/figures/simulation_workflow_formal.png}}
\caption{ Bimodal Simulation Workflow: Sequential execution of hydraulic flux calculation and graph-grammar state transformations.}
\label{fig:workflow}
\end{figure}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.45\textwidth]{results/figures/plot5_resilience_failure.png}}
\caption{ System Resilience ($R$) vs. Initial Failure Probability: Identifying the non-linear collapse threshold at 18\%.}
\label{fig:resilience}
\end{figure}

Beyond this threshold, the system enters a phase transition where network-wide inundation propagates catastrophically, reducing resilience to below 0.3 within 50 time steps.
...
To validate our framework, we compared its predictive accuracy against a "Uniform Flow" baseline (Figure \ref{fig:baseline}). The baseline model misses 40\% of flood events because it fails to account for structural transformations like pipe blockages and the resultant hydrograph rerouting.

\begin{figure}[htbp]
\centerline{\includegraphics[width=0.45\textwidth]{results/figures/plot9_baseline_comparison.png}}
\caption{ Methodology Comparison: Performance of the Proposed Graph Grammar Engine vs. Uniform Flow Baseline.}
\label{fig:baseline}
\end{figure}
